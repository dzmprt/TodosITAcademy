...
namespace Todos.FunctionalTests
{
    public class TodosApiTests : IClassFixture<CustomWebApplicationFactory<Program>>
    {
        ...
        [FixtureInlineAutoData(1, null)]
        [FixtureInlineAutoData(1, 1)]
        public async Task Get_Todos_Count_ReturnSuccessAndCorrectContentType(int? limit, int? offset)
        {
            // Arrange
            var client = _factory.CreateClient();
        ...
        public async Task Get_Todo_ReturnSuccessAndCorrectContentType()
        {
            ...
            // Act
            using (var requestMessage =
                   new HttpRequestMessage(HttpMethod.Get, $"/api/v1/Todos/{existTodoId}"))
            {
                requestMessage.Headers.Authorization =
                    new AuthenticationHeaderValue("Bearer", adminToken);
        ...
        public async Task Update_Todo_ReturnOkAndCorrectContentType()
        {
            ...
            var client = _factory.CreateClient();
            // Act
            using (var requestMessage = new HttpRequestMessage(HttpMethod.Put, $"/api/v1/Todos/{existTodoId}"))
            {
        ...
        [Fact]
        public async Task Update_TodoIsDone_ReturnOkAndCorrectContentType()
        {
            // Arrange
            var payload = new UpdateTodoIsDonePayload()
            {
                IsDone = true
            };
            ...
            var client = _factory.CreateClient();
            // Act
            using (var requestMessage = new HttpRequestMessage(HttpMethod.Patch, $"/api/v1/Todos/{existTodoId}/IsDone"))
            {
        ...
        public async Task Delete_User_ReturnOkAndCorrectContentType()
        {
            ...
            var client = _factory.CreateClient();
            // Act
            using (var requestMessage = new HttpRequestMessage(HttpMethod.Delete, $"/api/v1/Todos/{60}"))
            {
