...
namespace Todos.UnitTests.Tests.Commands.UpdateTodoIsDone
{
    ...
    public class UpdateTodoIsDoneCommandHandlerTests : RequestHandlerTestBase<UpdateTodoIsDoneCommand, GetTodoDto>
    {
        ...
        public UpdateTodoIsDoneCommandHandlerTests(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task Should_BeValid_When_UpdateTodosByAdmin(UpdateTodoIsDoneCommand command, Guid userId)
        {
            // arrange
        ...
        [Theory, FixtureInlineAutoData]
        public async Task Should_BeValid_When_UpdateTodosByClient(UpdateTodoIsDoneCommand command, Guid userId)
        {
            // arrange
            _currentServiceMok.SetupGet(p => p.CurrentUserId).Returns(userId);
        ...
        [Theory, FixtureInlineAutoData]
        public async Task Should_ThrowForbidden_When_UpdateTodosWithOtherOwnerByClient(UpdateTodoIsDoneCommand command, Guid userId)
        {
            // arrange
            _currentServiceMok.SetupGet(p => p.CurrentUserId).Returns(userId);
            _currentServiceMok.Setup(p => p.UserInRole(ApplicationUserRolesEnum.Client)).Returns(true);
