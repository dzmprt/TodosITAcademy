...
public class UpdateTodoIsDoneCommandHandlerTest : RequestHandlerTestBase<UpdateTodoIsDoneCommand, GetTodoDto>
{
    ...
    public async Task Should_BeValid_When_UpdateTodoIdDoneByAdmin(UpdateTodoIsDoneCommand command, Guid userId)
    {
        // arrange
    ...
    [Theory, FixtureInlineAutoData]
    public async Task Should_BeValid_When_UpdateTodoIdDoneByClient(UpdateTodoIsDoneCommand command, Guid userId)
    {
        // arrange
        _currentServiceMok.SetupGet(p => p.CurrentUserId).Returns(userId);
    ...
    [Theory, FixtureInlineAutoData]
    public async Task Should_ThrowForbidden_When_UpdateTodoWithOtherOwnerByClient(UpdateTodoIsDoneCommand command, Guid userId)
    {
        // arrange
        _currentServiceMok.SetupGet(p => p.CurrentUserId).Returns(userId);
        _currentServiceMok.Setup(p => p.UserInRole(ApplicationUserRolesEnum.Admin)).Returns(false);
    ...
    [Theory, FixtureInlineAutoData]
    public async Task Should_ThrowNotFound_When_UpdateToIsDonedoNotFound(UpdateTodoIsDoneCommand command)
    {
        // arrange
